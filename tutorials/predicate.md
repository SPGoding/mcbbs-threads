...
<!-- > 不重要的信息：这也是我反对将数据包中的 predicate 翻译为「谓词」的原因之一。分开翻译能够避免混淆数据包中的 predicate 文件与数据包的 predicate 文件中的 predicate 结构。因此，在本教程中，统一称 predicate 文件为「断言」，称断言文件中的部分通用结构称为「谓词」。 -->
...
- （对象）根标签。
    ...
    - （范围谓词）`dealt`：被防御前的伤害大小。
    - （范围谓词）`taken`：实际造成的伤害大小（可能被盔甲[size=0px].[/size]减免等）。
    - （实体谓词）`direct_entity`：伤害的直接来源实体（如弹射物）。
    - （实体谓词）`source_entity`：伤害的根本来源实体（如射箭的骷髅）。
    - （伤害类型谓词）`type`：伤害的类型所需满足的条件。 -->
